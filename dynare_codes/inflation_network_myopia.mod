// ============================================================
// Production Network Model with Myopia
...
//      mf=1 -> rational expectations; mf=0 -> complete myopia
//
// Reference: Minton & Wheaton (2023), Carvalho & Tahbaz-Salehi (2019)
// ============================================================
...
parameters alph      // Energy share in upstream production cost
           ...
           phi_DU    // Upstream goods share in downstream cost (IO coefficient)
...
// --- Calibration ---
alph    = 0.15;     // Energy share (upstream), from baseline
...
theta_U = 0.5;      // Upstream Calvo (avg. price duration 2 months; sensitivity: 0.3, 0.7)
phi_DU  = 0.10;     // IO coefficient (sensitivity: 0.05, 0.15)
mf      = 0.01;      // Myopia (sensitivity: 0, 1)
...
// --------------------------------------------------------
// (1) Aggregate production function (simplified from baseline)
// --------------------------------------------------------
gdp = employ - alph*tot;

// --------------------------------------------------------
// (2) Downstream marginal cost
//     rmc_D = relative price of upstream goods * phi_DU
//             + real wage * (1 - phi_DU)
//     nmc_D = rmc_D + cpi  (= phi_DU*cpi_U + (1-phi_DU)*(rwage+cpi))
// --------------------------------------------------------
rmc_D = phi_DU*(cpi_U - cpi) + (1-phi_DU)*rwage;
nmc_D = rmc_D + cpi;

// --------------------------------------------------------
// (3) Labour supply (aggregate, single labour market)
// --------------------------------------------------------
...
stoch_simul(irf=48, order=1) infl infl_U gdp tot nomint cpi_U;
